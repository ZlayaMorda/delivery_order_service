...
impl Config {
    ...
    pub fn init(mode: &str) -> Config {
        match mode {
            "dev" => { from_filename(".env.dev").expect("Failed to read .env.dev file"); }
            "prod" => { from_filename(".env.prod").expect("Failed to read .env.prod file"); }
            _ => { from_filename(".env.test").expect("Failed to read .env.test file"); }
        }
